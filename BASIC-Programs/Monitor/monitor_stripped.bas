10   T=0: D=0
20   DIM COMND$(16):DIM H$(4)

50   LET UADDR=&H8204

100  FUNCT=0: SRC=0: DESTN=0: LGTH=0
140  INPUT "Command: ";COMND$
150  FUNCT=ASC(COMND$)
160  IF FUNCT>90 THEN FUNCT=FUNCT-32

200  IF FUNCT=65 THEN GOTO 1000
210  IF FUNCT=67 THEN GOTO 2000
220  IF FUNCT=69 THEN GOTO 3000
230  IF FUNCT=73 THEN GOTO 4000
240  IF FUNCT=77 THEN GOTO 5000
250  IF FUNCT=81 THEN END
260  IF FUNCT=84 THEN GOTO 6000
270  ? "INVALID COMMAND": GOTO 100

1000 GOSUB 8100
1020 ? HEX$((DESTN+SRC));"h ";HEX$((DESTN-SRC));"h ";SRC-DESTN
1100 GOTO 100

2000 GOSUB 8000
2020 IF DESTN=SRC THEN GOTO 100
2030 FOR L=LGTH-1 TO 0 STEP -1
2040 POKE DESTN,PEEK(SRC): SRC=SRC+1: DESTN=DESTN+1
2050 NEXT
2100 GOTO 100

3000 GOSUB 8200
3020 IF LEN(COMND$)<8 THEN GOTO 9700
3030 DESTN=VAL(MID$(COMND$,8))
3040 DOKE UADDR,SRC
3050 ? USR(DESTN)
3100 GOTO 100

4000 GOSUB 8000
4020 IF DESTN=SRC THEN GOTO 100
4030 IF DESTN>SRC THEN GOTO 4200
4040 FOR L=LGTH-1 TO 0 STEP -1
4050 POKE DESTN,PEEK(SRC): SRC=SRC+1: DESTN=DESTN+1
4060 NEXT
4100 GOTO 100

4200 SRC=SRC+LGTH-1: DESTN=DESTN+LGTH-1
4220 FOR L=LGTH-1 TO 0 STEP -1
4230 POKE DESTN,PEEK(SRC): SRC=SRC-1: DESTN=DESTN-1
4240 NEXT
4300 GOTO 100

5000 GOSUB 8200
5020 ? HEX$(SRC);"h is ";HEX$(PEEK(SRC))
5030 INPUT "New";H$
5040 GOSUB 9000
5050 IF T>=0 AND T<=255 THEN POKE SRC,T
5200 GOTO 100

6000 GOSUB 8300
6020 FOR S=SRC TO SRC+LGTH-1 STEP 16
6030 ? HEX$(S);"h ";
6040 FOR L=0 TO 15
6050 ? " ";HEX$(PEEK(S+L));
6060 NEXT L
6070 ?
6080 NEXT S
6100 GOTO 100

8000 IF LEN(COMND$)<13 THEN GOTO 9700
8020 H$=MID$(COMND$,13)
8030 GOSUB 9000
8040 IF T<0 OR T>65536 THEN GOTO 9700
8050 LGTH=T

8100 IF LEN(COMND$)<11 THEN GOTO 9900
8120 H$=MID$(COMND$,8,4)
8130 GOSUB 9000
8140 IF T<0 OR T>65536 THEN GOTO 9900
8150 IF T>=32768 AND T<=65536 THEN T=T-65536
8160 DESTN=T

8200 IF LEN(COMND$)<6 THEN GOTO 9900
8220 H$=MID$(COMND$,3,4)
8230 GOSUB 9000
8240 IF T<0 OR T>65536 THEN GOTO 9900
8250 IF T>=32768 AND T<=65536 THEN T=T-65536
8260 SRC=T

8290 RETURN

8300 IF LEN(COMND$)<8 THEN GOTO 9700
8320 H$=MID$(COMND$,8)
8330 GOSUB 9000
8340 IF T<0 OR T>65536 THEN GOTO 9700
8360 LGTH=T

8390 GOTO 8200

9000 T=0: D=1
9030 FOR P=LEN(H$)-1 TO 0 STEP -1
9040 C=ASC(MID$(H$,D,1))
9050 D=D+1
9060 IF C>=48 AND C<=57 THEN C=C-48: GOTO 9100
9070 IF C>=65 AND C<=70 THEN C=C-55: GOTO 9100
9080 IF C>=97 AND C<=102 THEN C=C-87: GOTO 9100
9090 GOTO 9800
9100 T=T+INT(C*16^P+0.5)
9110 NEXT
9120 RETURN

9700 ? "NOT INT": RETURN
9800 ? "NOT HEX VAL": RETURN
9900 ? "NOT HEX ADDR": RETURN

