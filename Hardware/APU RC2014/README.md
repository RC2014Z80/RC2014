...
To assemble the APU Module, start with the two resistors. They will be 10k Ohm, although any value from 2.2k Ohm to 15k Ohm will work successfully. Locate them in the two positions indicated. They are not polarised.
...
Do not place a diode in the NMI diode location at this stage (or perhaps ever). _Advanced use only_.
...
The Am9511A benefits from being soldered directly to the board, because of the enhanced cooling effect of the PCB internal copper layers. However, in several years of operating Am9511A devices in sockets I have not experienced a failure, due to heat or otherwise.
...
The Am9511A APU Module is a passive device and can exist in any RC2014 system without impacting normal operation. It has a very narrow address range, and responds only to its base address (as set in the jumpers) and one address higher (e.g. the default addresses `0x42` and `0x43`). It can be available to accelerate arithmetic calculations as required.

Several methods of using the Am9511A APU Module exist. As a base platform a specific version of Microsoft (NASCOM) Basic has been created which integrates use of the APU into the Basic language.

Also, the Z88DK development platform includes Am9511A APU drivers and floating point library to support the use of the APU within C code or assembly code, on demand. Code generated by the Z88DK development platform can be loaded into the RC2014 using optionally the APU specific MS Basic (in Intel HEX format), CP/M (using XMODEM or otherwise), or other Intel HEX program loaders.
